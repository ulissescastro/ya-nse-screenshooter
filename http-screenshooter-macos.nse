...
categories = { "intrusive" } -- TODO: remove one or the other.
...
action = function( host, port )
  ...
  if string.match(port.service, 'https') then
      ...
      -- Mac users could use brew and install webkit2png
      -- brew install webkit2png
      cmd = "/usr/local/bin/webkit2png --fullsize --timeout=20 --ignore-ssl-check --filename=" .. screenshot_filename .. " " .. "https://" .. name .. ":" .. port.number .. " 2> /dev/null  >/dev/null"
  -- otherwise ALL try "http" as service
      ...
      cmd = "/usr/local/bin/webkit2png --fullsize --timeout=20 --ignore-ssl-check --filename=" .. screenshot_filename .. " " .. "http://" .. name .. ":" .. port.number .. " 2> /dev/null  >/dev/null"
      -- stdnse.print_debug(1, "VAR cmd = " .. cmd)
  end

  -- take shoot!
  local screenshot = os.execute(cmd)
  if screenshot then
      -- local response = {'screenshot: ' .. screenshot_filename }
      local response = screenshot_filename
